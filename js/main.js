...
import isOverAge from "./verify-age.js";
...
form.addEventListener("submit", (e) => {
  ...
  localStorage.setItem("login", JSON.stringify(answerList));
...
function verifyField(field) {
  ...
  erroType.forEach((erro) => {
    if (field.validity[erro]) {
      msg = mensagens[field.name][erro];
    }
  });
  const erroMsg = field.parentNode.querySelector(".mensagem-erro");
  const inputChecker = field.checkValidity();
